...
| [`scripts/claude-transcript.py`](scripts/claude-transcript.py) | User | Generate a Markdown transcript from a Claude Code session.<ul><li>Usage: `python scripts/claude-transcript.py --id <glob\|uuid\|latest> [output.md]`</li><li>Strips system-injected tags, collapses thinking and file changes in `<details>` blocks.</li><li>Use `--list` to browse sessions; `--project <path>` to target a different project.</li></ul> |
| [`scripts/codex-transcript.py`](scripts/codex-transcript.py) | User | Generate a Markdown transcript from a Codex session.<ul><li>Usage: `python scripts/codex-transcript.py --id <glob\|uuid\|latest> [output.md]`</li><li>Embeds images inline as base64 data URIs, collapses commentary in `<details>` blocks, attaches file diffs in collapsible `<details>` blocks.</li><li>Use `--list` to browse sessions by thread name.</li></ul> |
